...
void makeMuStaBkg(const TString infilename,           // input ntuple
  ...
  //TH1F *hAntiIso = new TH1F("hAntiIso", "hAntiIso", 60, 40, 200);  
  ...
  for(UInt_t ientry=0; ientry<intree->GetEntries(); ientry++) {
    ...
    if(effType==eHLTEff) {
      ...
      if (q1==q2) cout << category << endl;
      // INVERTED cut for background template selection


      if     (category==eMuMu2HLT) { continue; }
      else if(category==eMuMu1HLT) { continue; }
      else if(category==eMuMuNoSel) { 
	if(pfCombIso2>0.3*(lep2->Pt())) pass=kTRUE;
	else continue;
      }
      else if(category==eMuSta) { continue; }
      else { 
	if((pfCombIso2>0.3*(lep2->Pt())) && (q1==q2)) pass=kFALSE; 
	else continue;
      }
