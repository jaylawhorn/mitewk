...
ClassImp(mithep::SCNtuplerMod)

SCNtuplerMod::SCNtuplerMod(const char *name, const char *title):
  BaseMod        (name,title),
  fOutputFile    (0),
...
//--------------------------------------------------------------------------------------------------
SCNtuplerMod::~SCNtuplerMod()
{
  // Destructor
}	

//--------------------------------------------------------------------------------------------------      
void SCNtuplerMod::Begin()
{
}
...
>>>>>>> 73ecf38fbabb525fd7faab1c6457f2661f15464f
  ...
  fSCArr       = new TClonesArray("mithep::TPhoton");	assert(fSCArr);
  ...
  fEventTree->Branch("Photon",  &fSCArr);
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::SlaveTerminate()
{
  //
  // Save to ROOT file
  //
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::Terminate()
{
}

//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::BeginRun()
{
  if(HasHLTInfo() && fPrintTable) { GetHLTTable()->Print(); }
}

//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::EndRun()
{
}
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillMuon(const Muon *mu)
{
  assert(mu);
...
void SCNtuplerMod::FillMuon(const Track *mu)
{
  assert(mu);
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillElectron(const Electron *ele)
{
  assert(ele);
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillJet(const PFJet *jet)
{
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillSC(const SuperCluster *sc)
{
  TClonesArray &rSCArr = *fSCArr;
  assert(rSCArr.GetEntries() < rSCArr.GetSize());
  const Int_t index = rSCArr.GetEntries();  
  new(rSCArr[index]) TPhoton();
  TPhoton *pSC = (TPhoton*)rSCArr[index];
  ...
  pSC->passSpikeFilter    = kFALSE;
}

//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillPV(const Vertex *pv) 
{
...
<<<<<<< HEAD
	  ...
	  if(fTriggerObjNames1v[itrig].Length()>0 && fTriggerObjNames1v[itrig].CompareTo(to->ModuleName())==0) {
	    Bool_t match = kTRUE;
	    std::cout << "trigger object match v1!" << std::endl;
	    std::cout << "module name: " << to->ModuleName() << std::endl;
	    ...
	    std::cout << "deltaR: " << MathUtils::DeltaR(phi,eta,to->Phi(),to->Eta()) << std::endl;
	    std::cout << "trigger object eta: " << to->Eta() << " phi: " << to->Phi() << std::endl;
	    std::cout << "muon eta: " << eta << " phi: " << phi << std::endl;
...
<<<<<<< HEAD
	    ...
	    //std::cout << "pt threshold " << match << std::endl;
	    if(MathUtils::DeltaR(phi,eta,to->Phi(),to->Eta()) > hltMatchR) match=kFALSE;  // eta-phi matchin
	    //std::cout << "eta-phi matching " << match << std::endl;
...
ULong64_t SCNtuplerMod::MatchHLT(const Double_t pt, const Double_t eta, const Double_t phi)
{
  ULong64_t bits = 0;
...
//--------------------------------------------------------------------------------------------------
Bool_t SCNtuplerMod::IsConversion(const Electron *ele) 
{
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::separatePileUp(const Bool_t checkClosestZVertex)
{
  assert(fPFPileUp);
  assert(fPFNoPileUp);
...
//--------------------------------------------------------------------------------------------------
Float_t SCNtuplerMod::computePFIso(const Electron *ele, const Double_t ptMin, 
...
Float_t SCNtuplerMod::computePFIso(const Track *track, const Double_t ptMin, 
				   const Double_t extRadius, const Double_t intRadius,
				   const Int_t isoType)
{
  assert(track);
...
//--------------------------------------------------------------------------------------------------
Float_t SCNtuplerMod::computePUIso(const Electron *ele, const Double_t ptMin, 
                                   const Double_t extRadius, const Double_t intRadius)
{
  assert(fPFPileUp);
...
Float_t SCNtuplerMod::computePUIso(const Track *track, const Double_t ptMin, 
                                   const Double_t extRadius, const Double_t intRadius)
{
  assert(fPFPileUp);
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillGenWjets() 
{
  assert(fParticles);
  assert(fMCEvtInfo);
...
//--------------------------------------------------------------------------------------------------
void SCNtuplerMod::FillGenZjets() 
{
  assert(fParticles);
  assert(fMCEvtInfo);
