...
void fitRecoilZee(TString infilename,  // input ntuple
  ...
  fnamev.push_back("/scratch/klawhorn/EWKAnaR12a/Selection/Zee/ntuples/top_select.root"); isBkgv.push_back(kTRUE); 
  fnamev.push_back("/scratch/klawhorn/EWKAnaR12a/Selection/Zee/ntuples/ewk_select.root"); isBkgv.push_back(kTRUE);
  ...
  fcnPFu1sigma1->SetParameter(2,7);   fcnPFu1sigma1->SetParLimits(2,3,17);
  ...
  fcnPFu1sigma2->SetParameter(2,9.5);   fcnPFu1sigma2->SetParLimits(2,5,21);
  ...
  fcnPFu1sigma0->SetParameter(2,7);     fcnPFu1sigma0->SetParLimits(2,5,19); 
  ...
  fcnPFu2sigma1->SetParameter(2,5);      fcnPFu2sigma1->SetParLimits(2,2,17);
  ...
  fcnPFu2sigma2->SetParameter(2,9);      fcnPFu2sigma2->SetParLimits(2,5,21);
  ...
  fcnPFu2sigma0->SetParameter(2,7);      fcnPFu2sigma0->SetParLimits(2,5,19);        
  ...
  for(UInt_t ifile=0; ifile<fnamev.size(); ifile++) {
    ...
    for(Int_t ientry=0; ientry<intree->GetEntries(); ientry++) {
      ...
      if(sc1->Pt()        < PT_CUT  || sc2->Pt()        < PT_CUT)  continue;
      if(fabs(sc1->Eta()) > ETA_CUT || fabs(sc2->Eta()) > ETA_CUT) continue;
      ...
      u1_mva = ((dilep->Px())*(vU.Px()) + (dilep->Py())*(vU.Py()))/(dilep->Pt());  // u1 = (pT . u)/|pT|                                                                                                  
      u2_mva = ((dilep->Px())*(vU.Py()) - (dilep->Py())*(vU.Px()))/(dilep->Pt());  // u2 = (pT x u)/|pT|
	...
	if (mvaMet == kFALSE) {
          ...
          hPFu1Bkgv[ipt]->Fill(u1_mva);
          hPFu2Bkgv[ipt]->Fill(u2_mva);
        }
