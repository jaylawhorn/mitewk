...
Double_t getScaleCorr(const Double_t eta)
{
  ...
  else if(fabs(eta) < 1.4442) { return 1.0/1.00586; }
  else if(fabs(eta) < 2.0)    { return 1.0/1.00294; }
...
Double_t getResCorr(const Double_t eta)
{
  ...
  else if(fabs(eta) < 1.4442) { return 1.07197;   }
  else if(fabs(eta) < 2.0)    { return 0.893483;  }
...
//, (!) uncomment to perform corrections to recoil from W-MC/Z-MC
  ...
  // Signal MC
  fnamev.push_back("/scratch/klawhorn/EWKAnaR12a/Selection/Wenu/ntuples/we_select.root");   typev.push_back(eWenu);

  // EWK Backgrounds MC
  fnamev.push_back("/scratch/klawhorn/EWKAnaR12a/Selection/Wenu/ntuples/ewk_select.root");  typev.push_back(eEWK);
  ...
  //hPdfSignal->Scale(1/hPdfSignal->Integral());
