...
void selectAntiZmm(const TString conf,      // input file
  ...
  for(UInt_t isam=0; isam<samplev.size(); isam++) {
	...
	genBr = eventTree->GetBranch("Gen");
      ...
      if (isam != 0) {
        ...
        const TString baseName = ((TObjString*)(samp->fnamev[ifile].Tokenize("/."))->At(4))->String();
        TString puCorrFileName = puCorrDir + baseName + TString("-pu.root");
        cout << puCorrFileName << endl;
        TFile *puCorrFile = new TFile(puCorrFileName);
        puCorr = (TH1F*) puCorrFile->Get("hCorr");
	...
	muonArr->Clear();
	  ...
	  if(!passAntiMuonID(tag))               continue;  // lepton selection
	  ...
	  for(Int_t i2=0; i2<muonArr->GetEntriesFast(); i2++) {
	    ...
	    if (isam == 0)
              ...
              //Float_t puWeight = 1;
              //cout << npu << ", " << TMath::FloorNint(npu) << ", " << puCorr->GetBinCenter(TMath::FloorNint(npu)) <<  endl;
              Double_t puWeight = puCorr->GetBinContent(TMath::FloorNint(npu)+1);
              scale1fb = weight*puWeight;
              //scale1fb = weight;
            }
	    met      = info->pfMET;
